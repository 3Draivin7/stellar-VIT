...
export const AddGlasses: FC<AddGlassesProps> = ({ onClose }) => { 
  ...
  const foundWorker = workers.find(worker => worker.number === parseInt(number, 10));
    ...
    <> 
      ...
      <Modal isOpen={!showSuccessModal && !showErrorModal} onClose={onClose}> 
        <section className={styles.popup}> 
          <h1 className={styles.popup_title}>Добавление баллов для {number} {foundWorker !==undefined && foundWorker?.name.charAt(0).toUpperCase() + foundWorker?.name.slice(1)} {foundWorker !==undefined && foundWorker?.secondName.charAt(0).toUpperCase() + foundWorker?.secondName.slice(1)}</h1> 
      ...
      <Modal isOpen={showSuccessModal} onClose={() => navigate('/table')}> 
        <div  className={styles.popup}>
          <span className={styles.popup_title}>Баллы успешно добавлены</span></div> 
      </Modal> 

      {/* Модальное окно для ошибок */} 
      <Modal isOpen={showErrorModal} onClose={() => navigate('/table')}> 
        <div className={styles.popup}> <span className={styles.popup_title}>Произошла ошибка при добавлении баллов</span></div> 
      </Modal> 
    </> 
  ); 
}; 
