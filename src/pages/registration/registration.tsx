...
export const RegistrationForm = () => {
  ...
  const [errorRegistrationMessage, setErrorRegistrationMessage] = useState<string | null | undefined>('');
...
try{
      ...
      try {
        ...
        await dispatch(register({ name, email:number, password, secondName })).unwrap();
      } catch (error) {
        if (Object.keys(error).length === 0) {
          setIsOpenModal(true);
        } else {
          setErrorRegistrationMessage(error.message || 'Произошла ошибка при регистрации.');
        }
      } finally {
        setIsLoading(false);
      }
 ...
 <li className={styles.nav_menu_item}>
    ...
    <section className={styles.profile}>
      ...
      <Preloader/>
        ...
        <button className={styles.profile_button} type="submit">Зарегистрироваться</button>
      </form>
    </section>
