 ...
 <li className={styles.nav_menu_item}>
    ...
    <section className={styles.gifts}>
      <div className={styles.adm_container}>
        ...
        {lvl !== undefined && lvl > 1 && (
        <Link to='/gifts/add'>
        <button className={styles.add_gift}>Добавить подарок</button>
        </Link>
        )}
      </div>
      <ul className={styles.gifts_list}>
      {gifts && gifts.map((gifts) => (
        <li className={styles.gifts_item}>
                  {lvl !== undefined && lvl > 1 && (
          <button onClick={() => {setShowTrueModal(true); setDeleteWorker(gifts._id)}} className={styles.delete_button}>
            <img className={styles.delete_logo} src={deleteImg}/>
          </button>
                  )}
          <img src={gifts.link} alt="" className={styles.gifts_img}/>
          <h2 className={styles.gifts_name}>
            {gifts.name}
          </h2>
          <p className={styles.gifts_price}>{gifts.points} <span className={styles.size_price}>похвалюшек</span></p>
        </li>
      ))}
      </ul>
    </section>
...
<Modal isOpen={showSuccessModal} onClose={() => setShowSuccessModal(false)}>
      ...
      <Modal isOpen={showErrorModal} onClose={() => setShowSuccessModal(false)}> 
        <div className={styles.popup}> <span className={styles.popup_title}>Произошла ошибка при удалении подарка</span></div> 
      </Modal> 
