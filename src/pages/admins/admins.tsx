...
import { fetchUsers } from '../../services/slices/admins/allAdmins';
 ...
 <li className={styles.nav_menu_item}>
      {isAuthenticated ? (
        ...
        <div className={styles.header_table}>
          ...
          <h1 className={styles.header_table_title}>Таблица<span className={styles.size_header}>менеджеров</span></h1>
        </div>
        <div className={styles.table_leaders}>
          <ul className={styles.table_leaders_list}>
            ...
            <li className={lvl !== undefined && lvl > 0 ? styles.table_leaders_item_adm : styles.table_leaders_item}>
              <span className={styles.number}>{worker.email}</span><span className={styles.name}>{`${worker.name} ${worker.secondName}`}</span><span className={styles.glasses}></span>
              <span className={styles.adm_button_list}>
    <button onClick={() => {setShowTrueModal(true); setDeleteWorker(worker.email)}} className={styles.adm_button}>
...
<Modal isOpen={showSuccessModal} onClose={() => setShowSuccessModal(false)}>
      ...
      <Modal isOpen={showErrorModal} onClose={() => setShowSuccessModal(false)}> 
        <div className={styles.popup}> <span className={styles.popup_title}>Произошла ошибка при удалении менеджера</span></div> 
      </Modal> 
