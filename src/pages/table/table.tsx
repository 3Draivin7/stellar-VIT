...
export const Table: FC = () => {
  ...
  const workers = workerss.map((employee) => ({
    ...
    secondName:
      employee.secondName.charAt(0).toUpperCase() +
      employee.secondName.slice(1),
            ...
            <section className={styles.table}>
              <div className={styles.header_table}>
                ...
                <h1 className={styles.header_table_title}>
                  Таблица<span className={styles.size_header}>результатов</span>
                </h1>
              </div>
              <div className={styles.table_leaders}>
                {lvl !== undefined && lvl === 2 && (
                  <div className={styles.button_container}>
                    <Link
                      className={styles.table_leaders_list}
                      to='/table/addworkers'
                    >
                ...
                <ul className={styles.table_leaders_list}>
                  {worker &&
                    worker.map((worker) => (
                      ...
                      >
                        <span className={styles.number}>{worker.number}</span>
                        <span
                          className={styles.name}
                        >{`${worker.name} ${worker.secondName}`}</span>
                        <span className={styles.glasses}>{worker.points}</span>
      ...
      <Modal isOpen={showErrorModal} onClose={() => setShowSuccessModal(false)}>
        <div className={styles.popup}>
          ...
          <span className={styles.popup_title}>
            Произошла ошибка при удалении cотрудника
          </span>
        </div>
      </Modal>
